	...
	<script>
		...
		$(document).ready(function(){
			...
			function(data){
				...
				// Tracking Info.
				tbtrack = $(".markdown-body table")[3];
				data.track.forEach(function(rec){
					row = tbtrack.insertRow();
					row.insertCell().innerText = rec.uid;
					row.insertCell().innerText = rec.status;
					row.insertCell().innerText = rec.location;
					row.insertCell().innerText = rec.owner;
					row.insertCell().innerText = rec.operator;
					row.insertCell().innerText = rec.date;
					row.insertCell().innerText = rec.comment;
				});
			});
		});
	</script>
